...
export default function SinglePublications() {
    ...
    async function fetchOneData() {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/magazine/fetchOnemagazine/${magazine_slug}`
        );

        if (response.status === 200 && response.data.length > 0) {
          setMagazine(response.data[0]);
          setFetchError(false);
        } else {
          setFetchError(true);
        }
  ...
  return (
    <div className="w-full max-w-6xl mx-auto pt-20 lg:pt-32 xl:pt-40 px-4 lg:px-3 md:px-10 py-10 flex flex-col items-center">
      ...
      <h2 className="text-4xl mb-4 text-left bg-linear-to-r from-orange-600 via-orange-500 to-orange-200 bg-clip-text text-transparent">Tags</h2>

      <div className="flex flex-wrap gap-3">
        {magazine.magazine_tags &&
        typeof magazine.magazine_tags === "string" ? (
          magazine.magazine_tags.split(", ").map((tag: string, index: number) => (
            <span
              key={index}
